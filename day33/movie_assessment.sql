...
select * from movie where release_year =2019;
...
select * from movie where genre = "sci-fi" and rating > 8.5;

-- 6. List movies directed by either 'Christopher Nolan' or 'Martin Scorsese'.

select * from movie where director ="Christopher Nolan" or director ="Martin Scorsese" ;
...
select * from movie order by box_office_collection asc;
...
select * from movie order by rating desc limit 1 offset 2;
...
select * from movie where director like "s%";
...
select * from movie where rating between 7.5 and 8.5; 

-- 16. List all movies that belong to either the 'Action', 'Thriller', or 'Horror' genres.

select *from movie where genre in ("Action","Thriller","Horror");
